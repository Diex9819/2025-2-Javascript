...
function nombresConVocal (nombres){
    ...
    for (let v = 0; v < nombres.length; v++){
        let primeraLetra = nombres[v][0].toLowerCase();
        if (vocales.includes(primeraLetra)){
            resultado.push(nombres[v])
        }
    }
    return resultado;
...
async function main() {
    ...
    const nomb = [Diego, Fer, Cesar, Sol,  Luis];
    console.log (nombresConVocal(vocales));
